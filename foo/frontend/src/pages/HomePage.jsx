  ...
  const loadData = async () => {
    try {
      const [articlesRes, trendingRes, pollsRes, quizzesRes, lbRes, catRes] = await Promise.all([
        api.get('/articles?sort=latest&limit=7'),
        api.get('/articles?sort=popular&limit=4'),
        api.get('/polls?status=active'),
        api.get('/quizzes?status=active'),
  ...
  return (
    <div className="bg-white" data-testid="homepage">
      ...
      <section className="relative border-b border-jepang-black bg-jepang-black overflow-hidden">
        <div className="absolute inset-0 asanoha-bg" />
        <div className="relative px-4 md:px-8 lg:px-12 py-8 md:py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            ...
            <div className="hidden md:flex justify-end">
              <Link to="/register" className="jepang-btn-primary" data-testid="hero-register-btn">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-jepang-border p-5">
              ...
              <div className="space-y-0">
                {trending.slice(0, 4).map((article, idx) => (
                  <div key={article.id} className="flex gap-3 py-3 border-b border-jepang-border last:border-b-0">
                    ...
                    <div className="flex-1">
                      <Link to={`/articles/${article.slug}`} className="font-heading font-bold text-sm hover:text-jepang-red transition-colors line-clamp-2" data-testid={`trending-${article.slug}`}>
                        {article.title}
                      </Link>
      ...
      <section className="px-4 md:px-8 lg:px-12 py-12 bg-jepang-off-white">
        <div className="flex items-end justify-between mb-8 pb-3 border-b-2 border-jepang-black">
          ...
          <Link to="/articles" className="hidden md:inline-flex items-center gap-1 text-sm font-semibold uppercase tracking-wider hover:text-jepang-red transition-colors" data-testid="view-all-articles">
      ...
      <section className="px-4 md:px-8 lg:px-12 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-jepang-black p-6 hard-shadow-red">
            ...
            {polls.length > 0 ? (
              <div>
                <span className="jepang-badge mb-3">{polls[0].poll_type?.toUpperCase()}</span>
                <h4 className="font-heading font-bold text-2xl tracking-tight mb-3 mt-3">{polls[0].title}</h4>
                <p className="text-sm text-jepang-muted mb-4">{polls[0].total_votes || 0} votes</p>
                <Link to={`/polls/${polls[0].slug}`} className="jepang-btn-primary inline-block" data-testid="homepage-poll-cta">
                  Vote Now
                </Link>
              </div>
            ) : (
          ...
          <div className="bg-jepang-black text-white p-6">
            ...
            {quizzes.length > 0 ? (
              <div>
                ...
                <p className="text-sm text-zinc-400 mb-4">{quizzes[0].question_count || 0} questions • +10 PTS</p>
                <Link to={`/quizzes/${quizzes[0].slug}`} className="jepang-btn-primary inline-block" data-testid="homepage-quiz-cta">
                  Start Quiz
                </Link>
              </div>
            ) : (
      ...
      <section className="px-4 md:px-8 lg:px-12 py-12 bg-jepang-off-white">
        <div className="flex items-end justify-between mb-8 pb-3 border-b-2 border-jepang-black">
          ...
          <Link to="/leaderboard" className="hidden md:inline-flex items-center gap-1 text-sm font-semibold uppercase tracking-wider hover:text-jepang-red transition-colors" data-testid="view-full-leaderboard">
        ...
        <div className="bg-white border border-jepang-black">
          {leaderboard.length > 0 ? (
            leaderboard.slice(0, 5).map((entry, idx) => (
              <div key={entry.user_id} className="flex items-center gap-4 px-6 py-4 border-b border-jepang-border last:border-b-0" data-testid={`leaderboard-row-${idx}`}>
                ...
                <div className="w-10 h-10 bg-jepang-black text-white flex items-center justify-center font-bold">
                  {entry.display_name?.charAt(0).toUpperCase() || 'J'}
                </div>
                <div className="flex-1">
                ...
                <div className="text-right">
                  <p className="font-mono font-black text-xl text-jepang-red">{entry.weekly_points}</p>
