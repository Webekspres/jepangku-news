...
export default function PollDetailPage() {
  ...
  const loadPoll = async () => {
    try {
      const { data } = await api.get(`/polls/${slug}`);
      setPoll(data);
    } catch (e) {
      console.error(e);
      navigate('/polls');
    ...
    <div className="bg-white min-h-screen" data-testid="poll-detail-page">
      <div className="px-4 md:px-8 lg:px-12 py-12">
        <div className="max-w-3xl mx-auto">
          <Link to="/polls" className="inline-flex items-center gap-2 small-caps text-jepang-muted hover:text-jepang-red mb-6" data-testid="back-to-polls">
            <ArrowLeft size={14} /> Back to Polls
          </Link>

          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <span className={poll.poll_type === 'voting' ? 'jepang-badge-red' : 'jepang-badge-black'}>{poll.poll_type?.toUpperCase()}</span>
              <span className="flex items-center gap-1 jepang-badge text-jepang-red border-jepang-red">
                <Award size={10} strokeWidth={1.5} /> +{poll.points_reward || 5} PTS
              </span>
            </div>
          ...
          <div className="bg-white border border-jepang-black p-6 hard-shadow-red">
              ...
              {poll.options.map((option, idx) => {
                ...
                return (
                  <button
                    ...
                    disabled={voting}
                    className="w-full text-left relative overflow-hidden border border-jepang-border hover:border-jepang-black transition-colors disabled:opacity-50"
                    data-testid={`poll-option-${idx}`}
                  >
                    <div className="absolute inset-0 bg-jepang-red opacity-15 transition-all" style={{ width: `${percentage}%` }} />
                    <div className="relative flex items-center justify-between p-4">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-xs font-mono text-jepang-muted">{option.vote_count || 0} votes</span>
                        <span className="text-xl font-mono font-black text-jepang-red">{Math.round(percentage)}%</span>
                      </div>
                    </div>
                  </button>
                );
            ...
            <div className="mt-6 pt-4 border-t border-jepang-border flex items-center justify-between text-xs font-mono uppercase tracking-wider">
              ...
              {!user && (
                <Link to="/login" className="text-jepang-red font-bold">LOGIN TO VOTE & EARN POINTS</Link>
              )}
            </div>
          </div>
        </div>
