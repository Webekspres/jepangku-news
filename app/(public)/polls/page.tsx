...
export default function PollListPage() {
  ...
  const loadPolls = () => {
    fetch('/api/polls?status=ACTIVE').then((r) => r.json()).then((d) => {
    ...
    <div className="bg-white min-h-screen" data-testid="poll-list-page">
      <section className="border-b-2 border-[#0A0A0A] bg-[#F4F4F5]">
        <div className="px-4 mx-auto max-w-7xl py-12">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#D90429] mb-2">投票 / POLLS</p>
          <h1 className="font-heading font-black text-4xl sm:text-5xl lg:text-6xl tracking-tighter mb-3">Polling & Voting</h1>
          <p className="text-[#52525B] max-w-2xl">Suarakan pendapatmu dan ikuti aktivitas voting komunitas Jepangku!</p>
        </div>
      </section>

      <div className="px-4 mx-auto max-w-7xl py-12">
        {loading ? (
          <p className="text-center text-xs font-semibold uppercase tracking-[0.2em] text-[#52525B] py-12">Loading polls...</p>
        ) : polls.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
            {polls.map((poll: any) => {
              const totalVotes = poll.options?.reduce((sum: number, opt: any) => sum + (opt.voteCount || 0), 0) || 0;
              return (
                <Card key={poll.id} className="border border-[#0A0A0A] p-6" data-testid={`poll-card-${poll.slug}`}>
                  <CardContent className="p-0">
                    <div className="flex items-center justify-between mb-3">
                      <Badge variant={poll.pollType === 'VOTING' ? 'red' : 'black'}>
                        {poll.pollType?.toUpperCase()}
                      </Badge>
                      <span className="flex items-center gap-1 text-xs font-mono uppercase tracking-wider text-[#D90429] font-bold">
                        <Award size={12} strokeWidth={1.5} /> +{poll.pointsReward || 5} PTS
                      </span>
                    </div>
                    <h3 className="font-heading font-bold text-2xl tracking-tight mb-4">{poll.title}</h3>
                    {poll.description && <p className="text-sm text-[#52525B] mb-4">{poll.description}</p>}
                    <div className="space-y-2 mb-4">
                      {poll.options?.map((option: any, idx: number) => {
                        ...
                        return (
                          <button
                            key={option.id}
                            onClick={() => handleVote(poll.id, option.id, poll.slug)}
                            disabled={voting === poll.id}
                            className="w-full text-left border border-[#E4E4E7] hover:border-[#0A0A0A] transition-colors overflow-hidden"
                            data-testid={`poll-${poll.slug}-option-${idx}`}
                          >
                            <div className="relative p-3">
                              <Progress value={pct} className="absolute inset-0 h-full opacity-10" />
                              <div className="relative flex items-center justify-between">
                                <span className="font-semibold text-sm">{option.optionText}</span>
                                <div className="flex items-center gap-3">
                                  <span className="text-xs font-mono text-[#52525B]">{option.voteCount || 0}</span>
                                  <span className="text-sm font-mono font-bold text-[#D90429]">{pct}%</span>
                                </div>
                              </div>
                            </div>
                          </button>
                    ...
                    <div className="pt-3 border-t border-[#E4E4E7] flex items-center justify-between text-xs font-mono uppercase tracking-wider">
                      <span className="text-[#52525B] flex items-center gap-1">
                        <BarChart3 size={12} strokeWidth={1.5} /> {totalVotes} TOTAL VOTES
                      </span>
                      <Link href={`/polls/${poll.slug}`} className="text-[#D90429] hover:underline font-bold" data-testid={`poll-detail-${poll.slug}`}>
                        View Details →
                      </Link>
                    </div>
                  </CardContent>
          ...
          <div className="text-center py-24" data-testid="no-polls">
            <MessageSquare size={48} strokeWidth={1.5} className="mx-auto mb-4 text-[#52525B]" />
            <p className="font-heading font-bold text-2xl mb-2">No active polls</p>
            <p className="text-[#52525B]">Check back soon for new polls!</p>
          </div>
        )}
      </div>
    </div>
