...
export default function QuizListPage() {
  ...
  useEffect(() => {
    fetch("/api/quizzes?status=ACTIVE")
      .then((r) => r.json())
      .then((d) => {
      ...
      <section className="border-b-2 border-foreground bg-foreground text-white">
        <div className="px-4 mx-auto max-w-7xl py-12">
          ...
          <p className="text-zinc-300 max-w-2xl">
            Ikuti quiz tentang anime, manga, budaya Jepang, dan dapatkan poin!
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {quizzes.map((quiz: any) => (
              <Link
                ...
                className="group block bg-white border border-jepang-border hover:border-foreground transition-all"
