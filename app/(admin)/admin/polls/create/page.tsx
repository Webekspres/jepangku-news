...
export default function AdminCreatePoll() {
  ...
  const handleSubmit = async () => {
    ...
    if (questions.length < 1) { toast.error("Minimal 1 pertanyaan"); return; }

    for (let qi = 0; qi < questions.length; qi++) {
      const q = questions[qi];
      if (!q.questionText.trim()) {
        toast.error(`Pertanyaan ${qi + 1} belum diisi`); return;
      }
      if (q.options.length < 2) {
        toast.error(`Pertanyaan ${qi + 1} butuh minimal 2 opsi`); return;
      }
      ...
      <div className="px-4 mx-auto max-w-7xl py-8 space-y-8">
        ...
        <section className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button type="button" variant={form.poll_type === "POLLING" ? "black" : "outline"} size="sm"
                onClick={() => setForm({ ...form, poll_type: "POLLING" })} data-testid="type-polling">
                Polling
              </Button>
              <Button type="button" variant={form.poll_type === "VOTING" ? "default" : "outline"} size="sm"
                onClick={() => setForm({ ...form, poll_type: "VOTING" })} data-testid="type-voting">
                Voting
              </Button>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Label htmlFor="show-result" className="cursor-pointer">Tampilkan hasil sebelum vote</Label>
            </div>
          </div>
        </section>
        ...
        <section className="pt-6 border-t border-jepang-border space-y-4">
          ...
          {questions.map((q, qi) => (
            <Card
              key={qi}
              className="border border-jepang-border bg-jepang-off-white"
            ...
            >
              <CardContent className="p-5 space-y-4">
                ...
                {/* Question text */}
                <div className="space-y-1.5">
                  <Label className="text-xs">Teks Pertanyaan <span className="text-jepang-red">*</span></Label>
        ...
        <div className="pt-6 border-t border-jepang-border flex gap-3">
          <Button type="button" onClick={handleSubmit} disabled={loading} data-testid="create-poll-submit">
