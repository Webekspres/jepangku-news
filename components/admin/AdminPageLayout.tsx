      ...
      <section className="border-b border-jepang-border bg-jepang-navy text-white">
        <div className={cn(ADMIN_CONTENT_CLASS, "py-10 md:py-12")}>
          {backHref ? (
            <Link
              href={backHref}
              className="mb-4 inline-flex items-center gap-2 text-xs font-semibold text-zinc-300 transition-colors hover:text-jepang-orange"
            >
              <ArrowLeft size={14} />
              {backLabel}
            </Link>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="min-w-0">
              <p className="section-label mb-2 text-jepang-orange">{label}</p>
              <h1 className="font-heading font-black text-3xl tracking-tighter sm:text-4xl lg:text-5xl">
                {title}
              </h1>
              {subtitle ? (
                <p className="mt-2 max-w-2xl text-sm text-zinc-100/95">{subtitle}</p>
              ) : null}
            </div>
