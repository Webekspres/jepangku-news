...
export default function QuizListPage() {
  ...
  const loadQuizzes = async () => {
    try {
      const { data } = await api.get('/quizzes?status=active');
      setQuizzes(data || []);
    } catch (e) {
      console.error(e);
    } finally {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {quizzes.map((quiz) => (
              <Link
                ...
                className="group block bg-white border border-jepang-border hover:border-jepang-black transition-all"
                data-testid={`quiz-card-${quiz.slug}`}
              >
                {quiz.thumbnail_url ? (
                  <div className="aspect-video overflow-hidden bg-jepang-off-white">
                    <img src={quiz.thumbnail_url} alt={quiz.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
                ) : (
                ...
                <div className="p-5">
                  ...
                  <div className="pt-3 border-t border-jepang-border flex items-center justify-between text-xs font-mono uppercase tracking-wider">
                    <span className="text-jepang-muted">{quiz.question_count || 0} Q</span>
                    <span className="flex items-center gap-1 text-jepang-red font-bold">
                      <Award size={12} strokeWidth={1.5} /> +{quiz.points_reward || 10} PTS
                    </span>
                  </div>
                </div>
              </Link>
